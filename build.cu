...
public:
    ...
    bool getBatch(void *bindings[], const char *names[], int nbBindings) noexcept override
    {
        ...
        for (int i = 0; i < mBatchSize; i++)
        {
            ...
            process_input(img, mDeviceBatchData + i * offset);
        }
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        std::cerr << "用法: ./build [onnx_file_path] [可选: calib_dir calib_list_file]" << std::endl;
        std::cerr << "  仅 onnx_file_path: 构建 FP16 引擎" << std::endl;
        std::cerr << "  带校准参数: 构建 INT8 引擎" << std::endl;
        return -1;
    }
    // 命令行获取onnx文件路径、校准数据集路径、校准数据集列表文件
    char *onnx_file_path = argv[1];
    char *calib_dir = (argc >= 4) ? argv[2] : nullptr;
    char *calib_list_file = (argc >= 4) ? argv[3] : nullptr;
    bool use_int8 = (calib_dir != nullptr && calib_list_file != nullptr);
    ...
    {
        sample::gLogInfo << "设备不支持int8，改用FP16." << std::endl;
    ...
    else if (use_int8)
    {
        ...
        sample::gLogInfo << "使用INT8量化构建引擎." << std::endl;
    }
    else
    {
        config->setFlag(nvinfer1::BuilderFlag::kFP16);
        sample::gLogInfo << "使用FP16构建引擎." << std::endl;
    }
    ...
    // ========== 5. 序列化保存engine ==========
    std::ofstream engine_file("./weights/yolov26_e2e_seg.engine", std::ios::binary);
    ...
    std::cout << "Engine build success! Saved to ./weights/yolov26_e2e_seg.engine" << std::endl;
